document.addEventListener('DOMContentLoaded', function() {
    ...
    const quantityInputs = document.querySelectorAll('.quantity-input button');
    if (quantityInputs.length > 0) {
        quantityInputs.forEach(button => {
            button.addEventListener('click', function() {
                ...
                // If we're on the cart page and there's a form, trigger update
...
function transformProductData(array) {
  return array.map((product) => {
    return {
      ...
      rating: {
        rate: product.rating, // Using the product rating as "rate"
        count: product.reviews.length, // Counting the number of reviews as "count"
      },
    };
  });
}
